...
import { getAfterBlankLine, getBeforeBlankLine, getOnlyVariable, getOutputNewline } from '../features/output';
import { getQuote } from '../features/quote';
import { getRandomEmoji } from '../features/random-emoji';
import { getRandomColor } from '../features/random-color';
...
function getStatementGenerator(document: TextDocument) {
    ...
    if (!statement) {
        ...
        const isJs = JAVASCRIPT_ALIAS.includes(document.languageId);
        let template = '';

        if (isJs && color) {
            template = `${quote}%c${getRandomEmoji()}${getFileDepth(
                document
            )}$1/($2):${getOutputNewline()}${quote}, ${quote}color: ${color}${quote}$3`;
        } else {
            template = `${quote}${getRandomEmoji()}${getFileDepth(document)}$1/($2):${getOutputNewline()}${quote}$3`;
        }
